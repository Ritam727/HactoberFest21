...
ll solve(){
    ...
    ll col = 0;
    cin>>row>>col;
    ll selectedSquare = max(row, col);
    ll completed = (selectedSquare-1)*(selectedSquare-1);
    if(selectedSquare%2 == 0){
        ll result = completed;
        result += row;
        result += abs(col - selectedSquare);
        return result;
    }
    else{
        ll result = completed;
        result += col;
        result += abs(row - selectedSquare);
        return result;
    }
}
